		    ...
		    //check if molecules are too close with sr
                ...
                else
                {
                    if (verdict == 1)
                    {
                        ...
                        // Counteract the for-loop's own "i += BATCH_SIZE" increment that
                        // runs unconditionally after this iteration, so a reset here
                        // truly lands back at i==0 instead of overshooting past a
                        // per-rank budget (max_attempts/total_ranks) smaller than
                        // BATCH_SIZE, which would spuriously trip the attempt-limit check.
                        i = -BATCH_SIZE;
                    }
                }
                ...
                if (success_flag)
                {
                    // See the comment above the other "i = -BATCH_SIZE" reset: this
                    // must land on 0 *after* the for-loop's implicit i += BATCH_SIZE.
                    i = -BATCH_SIZE;
                    do {volume = normal_dist_ab(volume_mean, volume_std);} while(volume < 0.1);
                }
